...
// RUTAS CARRITO
router.get('/cart', csrfProtected, carrito.verCarrito);
...
// AUTH
router.get('/signin', csrfProtected, isNotLoggedIn, auth.signin);
router.post('/signin', csrfProtected, isNotLoggedIn, auth.login);
...
router.get('/signup', csrfProtected, isNotLoggedIn, auth.signup);
router.post('/signup', csrfProtected, isNotLoggedIn, auth.saveSignup);
router.get('/verifica', auth.verificaEmail);
router.post('/renew', isNotLoggedIn, auth.nuevoPass);

// VENTAS
router.post('/pagar', csrfProtected, isLoggedIn, venta.pagar);
